...
#[derive(Accounts)]
pub struct SetVaultUiStatus<'info> {
    #[account()]
...
pub fn handler(ctx: Context<SetVaultUiStatus>, is_active: bool) -> Result<()> {
    ctx.accounts.vault_account.is_active_from_ui = is_active;
    Ok(())
}
